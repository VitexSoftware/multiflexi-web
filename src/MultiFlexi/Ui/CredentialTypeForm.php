...
class CredentialTypeForm extends SecureForm
{
    ...
    public function __construct(\MultiFlexi\CredentialType $credtype, array $formProperties = [])
    {
        ...
        $credTypeRow1->addColumn(4, [
            new \Ease\TWB4\FormGroup(_('Credential type Name'), new \Ease\Html\InputTextTag('name', empty($credtype->name()) ? ($credtype->getPrototype() ? $credtype->getPrototype()->name() : '').' '.$companer->getRecordName() : $credtype->name())),
            new \Ease\TWB4\FormGroup(_('Owned by Company'), new CompanySelect('company_id', $companyId)),
            new \Ease\TWB4\FormGroup(_('Credential type UUID'), new \Ease\Html\InputTextTag('uuid', $credtype->uuid(), ['disabled'], _('Autogenerated'))),
        ]);
        $helperCol = $credTypeRow1->addColumn(6, new \Ease\TWB4\FormGroup(_('Credential Prototype'), new CredentialPrototypeSelect('prototype', [], (string) $credtype->getDataValue('prototype'))));
